...
\section{The \holoclean{} Framework}\label{sec:framework}
  ...
  The second step generates a graphical probabilistic model over all cells of the input database and the input signals.
  The model expresses the distribution of random variables for all cells in the dataset describing the uncertainty in the cell's values.
  ...
  First \holoclean generates \ddlog{} relations, which 
  
  \begin{itemize}
    \item summary
      \begin{itemize}
        \item each cell $\to$ random variable
        \item can take values of finite domain = domain of attribute
        \item repairing constraints express uncertainty over the values of noisy cells
        \item probabilistic graphical model that describes the distribution of random variables
        \item $\to$ factor graph
      \end{itemize}
    \item compilation process
      \begin{enumerate}
      ...
      \end{enumerate}
    \item \ddlog{} relations
      \begin{itemize}
        \item all from dataset
        \item syntax: $t$ identifies tuples in $D$, $a$ identifies attributes in $D$
        \item relations in table~\ref{tab:relations}
      \end{itemize}
    \item Inference Rules
      \begin{itemize}
        \item generation of categorical random variable for each cell.
        \item minimality constraint
  ...
  \begin{table}
    ...
    \label{tab:relations}
    \begin{tabular}{rlp{0.5\linewidth}@{}}
      \toprule
      & Relation & Description\\
      \midrule
      1 & \ddrule{Tuple(t)} & Relation of all tuple identifiers.\\
      2 & \ddrule{InitValue(t,a,v)} & Maps a cell $t.a$ to its initial value in the input dataset.\\
