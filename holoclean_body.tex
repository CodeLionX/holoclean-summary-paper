...
\section{The \holoclean{} Framework}\label{sec:framework}
  ...
  \begin{itemize}
    \item The second input signal are \glspl{ic}.
    \item they connect cells and therefore also random variables
    \item In a later step \holoclean{} uses Gibbs sampling~\cite{gibbssampling} to perform inference.
    \item It iterates over the random variables of the factor graphs and while keeping all other variables fixed samples a single one from its conditional distribution.
    \item This process is computationally expensive.
    \item But if there are only independent random variables in the factor graph Gibbs sampling requires only $O(n \log n)$ steps to converge.
    \item All inference rules we already introduced do not cause any dependencies between random variables, because there is only one variable in the head of those rules.
    \item That is the reason \holoclean{} does not encode \glspl{ic} with multiple random variables in the head of the inference rules.
    \item Under the two following assumptions it can approximate the inference rules for \glspl{ic}:
      \begin{enumerate}
        \item more correct cells than erroneous ones, already used for minimality prior
        \item each violation of an \gls{ic} can be fixed by changing one of the involved cells
      \end{enumerate}
    \item For each denial constraint \holoclean{} generates multiple inference rules containing one of the involved cell's random variables in the head of the rule and the predicate constraints and the other cell's value mappings in the body.
    \item The quantifier of denial constraints ($\forall t_1, t_2$) is converted to a self-join of relation \ddrule{Tuple}.
  \end{itemize}
