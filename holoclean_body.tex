...
\section{Conclusion}\label{sec:conclusion}
  \begin{itemize}
    \item recap intro: different approaches use single input signal
    \item \holoclean{} uses holistic approach across multiple inputs
    \item leads to 2x better accuracy on average, compared to state-of-the-art
    \item does take longer, but justified by improvement
    \item limited by detection approaches used at the beginning
    \item future research direction: use semi-supervised learning on marginal probabilities, understand when \glspl{ic} can be relaxed to independent features on random variables and when not, how to provide theoretical guarantees
  \end{itemize}